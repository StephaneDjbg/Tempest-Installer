...
4. **Frequency**: Set the target frequency found with CubicSDR/GQRX
5. Press **Start** to begin signal capture
6. Refer to section 4.3 for detailed TempestSDR parameter configuration and correlation window fine-tuning
...
4. Press **Start** to begin signal capture
5. Refer to section 4.3 for detailed TempestSDR parameter configuration and correlation window fine-tuning
#### For Windows Users:
...
4. Configure ExtIO parameters in the dialog window:
   ...
   * **For USRP**: Set gain to mid-range (around 30-40 dB), change the antenna to the one you are using (RX2 is recommended for USRP B205mini, you just want to listen to the signal, not transmit)
...
3. **Low-pass filter (Lpass)**: 
   ...
   * Once you see a stable image, you can slide it further to the right to improve quality
4. **Correlation windows and fine-tuning**: This is the most critical indicator for successful image capture. Monitor both the framerate detection and video height detection windows carefully:
   
   **Framerate Detection Window**: Look for two clear spikes centered around your target framerate (e.g., 60 Hz for 1920x1080@60Hz). This indicates proper horizontal synchronization.
   
   **Video Height Detection Window**: Should display clear, distinct maximum peaks rather than scattered spikes throughout. Clean maximums indicate proper vertical synchronization.
   
   When you achieve the perfect scenario shown below, you can be confident of obtaining a high-quality image capture.

<div align="center">
<img src="captures/autocorrelationwindow.png" alt="Autocorrelation Window" width="600"/>
<br/>
<em>Perfect autocorrelation scenario: Clear dual spikes in framerate detection and distinct maximums in height detection indicate optimal signal capture and high-quality image recovery.</em>
</div>

**Correlation Window Fine-Tuning Process:**
1. **Initial Auto-Detection**: When you see a promising signal scenario in the correlation windows, click the **Aut** button. This will automatically select the maximum peaks in both correlation windows as a starting point.

2. **Manual Cursor Adjustment**: After auto-detection, you often need to manually fine-tune the cursors in both correlation windows:
   * **Watch the video reconstruction area**: As you move the cursors, observe how the reconstructed image quality changes in real-time
   * **Framerate window cursors**: Adjust to align with the clearest, strongest peaks - this affects horizontal sync
   * **Height detection cursors**: Move to capture the most distinct maximums - this affects vertical sync and image stability
   * **Small movements matter**: Even tiny cursor adjustments can dramatically improve image clarity and stability

3. **Iterative Process**: 
   * Move a cursor slightly and observe the image reconstruction
   * If the image improves, continue fine-tuning in that direction
   * If it degrades, move back and try the other direction
   * Switch between both correlation windows to optimize both horizontal and vertical sync

**Frequency Fine-Tuning Process:**
- Start by scanning the 300-500 MHz range to locate your target frequency
- Once you identify a promising frequency candidate, fine-tune in small increments (±0.1-0.5 MHz)
- Watch the correlation windows - avoid frequencies that show scattered spikes across the height detection
- The ideal signal will produce clean, organized patterns in both correlation windows
...
2. Click the **Aut** button when you see a promising signal scenario - this will automatically select the maximum peaks in the correlation windows
3. **Manual fine-tuning**: You may need to manually adjust the cursors in both correlation windows based on what you observe in the video reconstruction area:
   * Move the cursors to align with the clearest peaks
   * Monitor the reconstructed image quality as you adjust
   * Small cursor movements can significantly improve image clarity
4. Adjust other parameters while monitoring the display
